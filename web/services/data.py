...
def discover_ytm_playlists() -> dict[str, Any]:
    """Scan the YTM library and classify autogenerated playlists.

    Returns {"candidates": [...]} or {"error": "..."} on failure.
    """
    from src.playlist.discover import discover_playlists

    settings = _get_settings()
    if settings is None:
        return {"error": "Settings not configured", "candidates": []}
...
def list_tracked_playlists() -> dict[str, Any]:
    """Classify cached playlists offline (no YTM call) for the Playlists tab.

    Returns {"candidates": [...]} where every entry is tracked.
    """
    from src.playlist.discover import classify_playlist

    pc = load_playlist_cache()
    settings = _get_settings()
    custom_names = {c["name"] for c in load_custom_playlists_config()} if settings else set()
    out: list[dict[str, Any]] = []
    for name, entry in pc._cache.items():
        if not isinstance(entry, dict) or not entry.get("id"):
            continue
        category = "unknown"
        if settings is not None:
            category, _autogen, _conf = classify_playlist(name, "", settings, custom_names)
        out.append({"title": name, "id": str(entry["id"]), "category": category, "tracked": True})
    out.sort(key=lambda x: (x["category"], x["title"].lower()))
    return {"candidates": out}


def track_playlists_in_cache(items: list[dict[str, Any]]) -> int:
    """Write selected playlist IDs into the cache (IDs only). Returns count."""
    pc = load_playlist_cache()
    n = 0
    for it in items:
        ...
        if name and pid:
            pc.track_id(name, pid)
            n += 1
    return n


def delete_ytm_playlist(playlist_id: str, name: str | None = None) -> dict[str, Any]:
    """Delete a playlist from YouTube Music and drop its cache entry."""
    pc = load_playlist_cache()
    if not name:
        for pl_name, entry in pc._cache.items():
...
def prune_old_weeklies_ytm() -> dict[str, Any]:
    """Delete old weekly playlists on YTM, keeping the configured count."""
    from src.playlist.weekly import _prune_old_weeklies

    settings = _get_settings()
    if settings is None:
        return {"error": "Settings not configured", "deleted": []}
