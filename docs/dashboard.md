...
??? example "Screenshot: Playlists tab"
...
- **Preview sync (dry run)** - A **Preview** button (flask icon) sits to the left of the **Run** button in the sync console, and next to each custom playlist card's **Sync** button. Clicking it resolves tracks as usual but never touches YouTube Music; instead it opens a preview summarising, per playlist, the tracks that would be added (with match score and play count where available) and removed, plus whether the playlist would be reordered. Preview is a one-off action, not a mode - the plain **Run**/**Sync** buttons always apply changes immediately, and there is no state to remember or reset. The **Sync Multiple...** dialog has its own per-run dry-run toggle. Click **Apply Sync** in the preview to run the same sync for real. From the CLI, set `SYNC_DRY_RUN=1` (see [CLI Install](cli-install.md#preview-dry-run)).
