...
| `playlists`        | `playlists.png`          | Playlists tab (discover/track/prune autogenerated playlists) |
...
3. **Route stubs**  Playwright intercepts the volatile JSON endpoints
   ...
   `/api/setup/status`, `/api/settings`, `/api/playlists/tracked`,
   `/api/playlists/discover`) and serves canned payloads. This
